...
<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-6 flex justify-between items-center">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-600">Información completa del producto</p>
        </div>
        <a href="{{ route('productos.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 font-medium transition">
        ...
        <div class="md:flex">
            ...
            <div class="md:w-2/3 p-8">
                ...
                <div class="space-y-4 mb-6">
                    <div>
                        <h3 class="text-sm font-medium text-gray-500 uppercase tracking-wider mb-1">ID</h3>
                        <p class="text-gray-900">{{ $producto->id }}</p>
                    </div>

                    @if($producto->descripcion)
                    <div>
                        <h3 class="text-sm font-medium text-gray-500 uppercase tracking-wider mb-1">Descripción</h3>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <h3 class="text-sm font-medium text-gray-500 uppercase tracking-wider mb-1">Precio Unitario</h3>
                            <p class="text-gray-900 font-semibold">${{ number_format($producto->precio, 2) }}</p>
                        </div>
                        <div>
                            <h3 class="text-sm font-medium text-gray-500 uppercase tracking-wider mb-1">Cantidad en Stock</h3>
                            <p class="text-gray-900 font-semibold">{{ $producto->stock }} unidades</p>
                        </div>
                    </div>

                    <div class="grid grid-cols-2 gap-4 pt-4 border-t border-gray-200">
                        <div>
                            <h3 class="text-sm font-medium text-gray-500 uppercase tracking-wider mb-1">Creado</h3>
                            <p class="text-gray-900 text-sm">{{ $producto->created_at->format('d/m/Y H:i') }}</p>
                        </div>
                        <div>
                            <h3 class="text-sm font-medium text-gray-500 uppercase tracking-wider mb-1">Última Actualización</h3>
                            <p class="text-gray-900 text-sm">{{ $producto->updated_at->format('d/m/Y H:i') }}</p>
                        </div>
                    </div>
                </div>
